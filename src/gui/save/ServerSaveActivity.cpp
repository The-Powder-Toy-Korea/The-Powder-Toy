...
void ServerSaveActivity::ShowRules()
{
	String rules =
		...
		"\bt3. 어떤 방법으로도 투표를 요구하는 것 또한 옳지 않은 행동입니다.\bw 이러한 행위를 하는 세이브는 수정될 때까지 비공개될 것입니다. 아래는 이 규칙에 해당되는 행위의 예시입니다.\n"
		   "- \"좋아요\" 또는 \"싫어요\" 투표를 암시하는 기호들. 초록색 화살표나 투표를 할 것을 요구하는 것이 이 규정에 어긋납니다.\n"
		   "- 투표를 받기 위한 술책들. 이러한 행위는 이를테면 '100 표를 받으면 더 개선된 것을 내놓겠습니다.'와 같은 총 투표수에 대하여 무언가를 내놓겠다는 것을 말합니다. 우리는 이것을 '투표수 파밍'라고 부르고 이러한 행위는 허용되지 않습니다.\n"
		   "- 세이브 사용에 대한 답례로 투표를 요구하거나 기타 다른 이유로 투표를 요구하는 것 또한 금지입니다.\n"
		"\bt4. 스팸 행위는 금지입니다.\bw 앞서 말하였듯이, 우리가 스팸으로 여기는 것은 정확한 정의가 없습니다. 아래는 스팸으로 간주될 수 있는 행위의 예시입니다.\n"

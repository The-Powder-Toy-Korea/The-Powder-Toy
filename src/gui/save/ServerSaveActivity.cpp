...
void ServerSaveActivity::ShowPublishingInfo()
{
	String info =
		...
		"\n"
		"어떤 세이브를 즉시 다시 저장하려면 세이브를 열고 저장 버튼의 왼쪽 부분의 \bt'현재 시뮬레이션을 재업로드'\bw 버튼을 누르십시오. 작품의 설명이나 공개 상태를 바꾸고 싶다면 오른쪽 부분의 \bt'시뮬레이션 속성 변경'\bw 버튼을 누르십시오. 단, 이때 세이브의 이름을 바꿀 경우 이 세이브는 완전히 새로운 세이브로 저장되어 기존의 댓글, 투표, 태그가 사라지게 됩니다.\n"
...
void ServerSaveActivity::ShowRules()
{
	String rules =
		...
		"\n"
		"어떤 행위가 규칙에 맞는지 어긋나는지 묻고 싶다면 언제나 관리자에게 연락하세요.";

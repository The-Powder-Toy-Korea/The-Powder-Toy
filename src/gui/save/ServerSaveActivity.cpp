...
void ServerSaveActivity::ShowPublishingInfo()
{
	String info =
		...
		"\n"
		"\bt공개된 세이브\bw는 '날짜순 정렬'에 표시되어 많은 사람이 볼 수 있습니다. 이러한 세이브는 The Powder Toy 홈페이지에 있는 당신의 프로필에 표시되는 평균점수에 영향을 줄 것입니다. 사람들이 당신의 작품을 보고 댓글을 달고 투표하게 하려면 작품을 공개하십시오.\n"
		"\bt비공개된 세이브\bw는 '날짜순 정렬'에서 보이지 않습니다. 이것은 당신의 평균 점수에 영향을 주지 않습니다. 다만, 세이브 ID를 알고 있는 사람은 이러한 작품을 볼 수 있으므로 아예 공개되지 않는 것은 아닙니다. 당신은 특정한 사람들에게만 이러한 세이브를 보여주기 위해 세이브 ID를 배포할 수 있습니다.\n"
		"\n"
		"어떤 세이브를 즉시 다시 저장하려면 세이브를 열고 저장 단추의 왼쪽 부분의 \bt'현재 시뮬레이션을 재업로드'\bw 단추을 누르십시오. 작품의 설명이나 공개 상태를 바꾸고 싶다면 오른쪽 부분의 \bt'시뮬레이션 속성 변경'\bw 단추을 누르십시오. 단, 이때 세이브의 이름을 바꿀 경우 이 세이브는 완전히 새로운 세이브로 저장되어 기존의 댓글, 투표, 태그가 사라지게 됩니다.\n"
		"공개되지 않은 세이브를 완성한 다음 공개하고 싶거나, 현재 공개된 세이브를 비공개로 바꾸고 싶다면, 세이브를 열고 '시뮬레이션 속성 변경' 단추을 누르고, 공개 상태를 변경하면 됩니다. 세이브 검색 화면의 '내 세이브' 메뉴를 통하여 세이브를 선택한 뒤, 하단의 \bt'비공개로 전환' 또는 '삭제'\bw 단추을 통하여서도 해당 작업을 할 수 있습니다.\n"
		"어떤 세이브가 만들어진 지 일주일이 되지 않았고 빠른 속도로 인기를 얻는다면 자동으로 \bt프론트페이지\bw에 올라가게 됩니다. 공개된 세이브만이 프론트페이지에 등록됩니다. 관리자들도 매우 가끔씩 세이브를 프론트페이지에 올려놓습니다. 관리자들은 또한 규정을 어기거나 적절하지 않다고 생각하는 세이브를 프론트페이지에서 내릴 수 있습니다.\n"
		"한 번 세이브를 만들었다면 세이브를 다시 저장하는 데에는 제한이 없습니다. 짧은 기간의 \bt세이브 편집 기록\bw이 남으므로, 세이브 검색 화면에서 세이브를 마우스 오른쪽 클릭한 뒤 '편집 기록 보기'를 통하여 편집 기록을 볼 수 있습니다. 이것은 실수로 저장을 했거나 과거 버전으로 돌아가고 싶을 때 쓰일 수 있습니다.\n"
		;
...
void ServerSaveActivity::ShowRules()
{
	String rules =
		...
		"\bw아래 규칙들은 The Powder Toy 커뮤니티에서 활동할 때에 반드시 지켜야 할 규칙입니다. 이 규칙들은 스태프들에 의해 집행되며, 이러한 규정을 어기는 경우 다른 회원이 우리 스태프들에게 알릴 것입니다. 제1장은 세이브, 댓글, 포럼, 그리고 기타 커뮤니티의 다른 영역에서의 규칙입니다.\n"
		"\n"
		...
		"\bt2. 스팸 행위는 금지입니다.\bw 여기서 '스팸'이 항상 여기의 정의에 맞지는 않을 수도 있지만, 대부분의 경우 어떤 행동이 '스팸'인지 아닌지 명백하게 알아차릴 수 있을 것입니다. 아래는 '스팸'으로 간주되어 글 따위가 숨겨지거나 지워질 수 있는 행위입니다:\n"
		   "- 많은 수의 같은 주제를 가진 스레드를 포럼에 올리는 경우. 게임에 대한 피드백이나 제안은 최대한 모아서 올려 주십시오.\n"
		   ...
		   "- '+1' 등의 짧은 대답을 한 스레드에 올리는 경우. 이런 방법으로 지속적으로 스레드를 목록 위로 올릴 경우 다른 대답을 찾기 어려워집니다. 당신이 이 스레드를 지지함을 표현하는 '+1' 단추가 이미 있으니, 대답 기능은 건설적인 피드백에 사용해 주십시오.\n"
		...
		"\bt3. 욕설은 자제해 주십시오.\bw 욕설이 포함된 댓글이나 세이브는 지워질 수 있습니다. 다른 언어로 욕설을 사용하는 것도 이 규칙에 포함됩니다.\n"
		"\bt4. 성적으로 노골적이거나, 불쾌하거나, 기타 부적절한 것들을 업로드하지 마십시오.\bw\n"
		...
		"\bt5. The Powder Toy와 직접적으로 관련되지 않은 게임이나 사이트, 기타 다른 것을 광고하지 마십시오.\bw\n"
		...
		"\bt8. 관리자의 결정이나 문제점에 대한 내용은 쓰지 마십시오.\bw 이용 정지나 콘텐츠 삭제에 관한 이의가 있다면 메시지 시스템을 이용해서 관리자에게 연락하십시오. 이 외 관리자의 결정에 관한 내용에 관한 글은 자제하여 주십시오.\n"
		"\bt9. 관리자의 일에 참견하는 것은 삼가 주십시오.\bw 결정을 하는 것은 관리자입니다. 이용자들은 이용 정지가 되거나 규정 위반에 해당될 수 있는 일을 자제해야 합니다. 만약 어떤 일이 규정을 어길 것 같거나 확실하지 않다면 '신고' 단추을 통해서 알리거나 우리 홈페이지의 메시지 시스템을 통해 물어볼 것을 권장합니다.\n"
		...
		"\bt5. 성적으로 노골적이거나 기타 다른 부적절한 콘텐츠를 올리는 것은 삼가 주십시오.\bw 이러한 세이브는 삭제되고 이용 정지 처분이 내려질 것입니다.\bw\n"
		...
		"\bt7. 로고나 표지판은 최소한으로 사용해 주십시오.\bw 이러한 세이브는 프론트페이지에서 삭제될 것입니다. 아래는 이 규칙이 제한하는 것들입니다:\n"
		   "- 로고 남용\n"
		   "- 특별한 의도 없는 표지판의 사용\n"
		   "- 가짜 업데이트 또는 알림 표지판\n"
		   "- 관련이 없는 세이브로 링크하는 행위\n"
		"\bt8. 주제에 어긋나거나 부적절한 태그를 사용하지 마십시오.\bw 태그는 검색 기능을 개선하기 위해 존재하는 기능입니다. 태그들은 일반적으로 세이브를 설명하는 1개의 단어로만 이루어져 있습니다. 문장형 태그 또는 주관적인 태그는 삭제될 것입니다. 부적절하거나 모욕적인 태그를 올리는 것은 이용 정지 처분의 대상이 될 수 있는 행위입니다.\n"
		...
		"\bt10. 신고 시스템을 남용하지 마십시오.\bw '나쁜 세이브' 등의 이유나 의미 없는 내용으로 신고하면 우리의 시간이 의미 없이 쓰이게 됩니다. 규정 위반인 것 같거나 커뮤니티에서 사건사고가 일어나지 않는다면 신고를 하지 말아주십시오. 세이브의 내용이 규칙을 위반하거나 커뮤니티에 문제를 일으키는 것 같다면 신고하세요! 여러분이 좋은 의도로 신고한다면 여러분이 이용 정지될 일은 절대 없습니다.\n"
		"\bt11. 어떤 세이브를 프론트페이지에서 내려달라고 하는 행위는 삼가 주십시오.\bw 세이브가 규칙을 어기지 않는 이상, 그 세이브는 프론트페이지에 머무를 것입니다. 이 규정은 아트 세이브에도 예외가 아니므로, 아트 세이브에도 이러한 요구는 하지 말아주십시오.\n"
		"\n"
		"\bo섹션 R: 기타\n"

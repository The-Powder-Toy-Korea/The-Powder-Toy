...
void ServerSaveActivity::ShowRules()
{
	String rules =
		...
		"\bt2. 스팸 행위는 금지입니다.\bw 여기서 '스팸'이 항상 여기의 정의에 맞지는 않을 수도 있지만, 대부분의 경우 어떤 행동이 '스팸'인지 아닌지 명백하게 알아차릴 수 있을 것입니다. 아래는 '스팸'으로 간주되어 글 따위가 숨겨지거나 지워질 수 있는 행위입니다:\n"
		   ...
		   "- '+1' 등의 짧은 대답을 한 스레드에 올리는 경우. 이런 방법으로 지속적으로 스레드를 목록 위로 올릴 경우 다른 대답을 찾기 어려워집니다. 당신이 이 스레드를 지지함을 표현하는 '+1' 단추가 이미 있으니, 대답 기능은 건설적인 피드백에 사용하십시오.\n"
		...
		"\bt8. 관리자의 결정이나 문제점에 대한 내용은 쓰지 마십시오.\bw 이용 정지나 콘텐츠 삭제에 관한 이의가 있다면 메시지 시스템을 이용해서 관리자에게 연락하십시오. 이 외 관리자의 결정에 관한 내용에 관한 글은 자제하십시오.\n"
		"\bt9. 관리자의 일에 참견하는 것은 삼가십시오.\bw 결정을 하는 것은 관리자입니다. 이용자들은 이용 정지가 되거나 규정 위반에 해당될 수 있는 일을 자제해야 합니다. 만약 어떤 일이 규정을 어길 것 같거나 확실하지 않다면 '신고' 단추을 통해서 알리거나 우리 홈페이지의 메시지 시스템을 통해 물어볼 것을 권장합니다.\n"
		...
		"\n"
		"\bo섹션 G - 게임 내 규칙\n"
		"\bw섹션 G은 게임 내 행위에 대해 중점적으로 다룹니다. 섹션 S 역시 게임 내 행위에 적용되지만 아래 규칙은 인게임 커뮤니티에서의 활동에 대해 더욱 중점적으로 다룹니다.\n"
		...
		"\bt2. 자기 작품에 자신이 직접 투표하거나, 투표를 조작하는 것은 금지되어 있습니다.\bw 투표 조작이란 많은 계정을 만들어서 자신이나 타인의 세이브에 투표하는 것을 말합니다. 이 규정은 엄격하게 적용되므로, 이의 신청이 거의 받아들여지지 않음을 명심하십시오. 당신과 다른 계정이 같은 세이브에 투표하지는 않는지 확인하십시오. 다중 계정은 모두 영구적으로 이용이 정지될 것이며, 주 계정은 일시적으로 이용이 정지되고 영향을 받은 모든 세이브가 삭제될 것입니다.\n"
		...
		"\bt5. 성적으로 노골적이거나 기타 다른 부적절한 콘텐츠를 올리는 것은 삼가십시오.\bw 이러한 세이브는 삭제되고 이용 정지 처분이 내려질 것입니다.\bw\n"
		...
		"\bt7. 로고나 표지판은 최소한으로 사용하십시오.\bw 이러한 세이브는 프론트페이지에서 삭제될 것입니다. 아래는 이 규칙이 제한하는 것들입니다:\n"
		   "- 로고 남용\n"
		   "- 특별한 의도 없는 표지판의 사용\n"
		   "- 가짜 업데이트 또는 알림 표지판\n"
		   "- 관련이 없는 세이브로 링크하는 행위\n"
		...
		"\bt10. 신고 시스템을 남용하지 마십시오.\bw '나쁜 세이브' 등의 이유나 의미 없는 내용으로 신고하면 우리의 시간이 의미 없이 쓰이게 됩니다. 규정 위반인 것 같거나 커뮤니티에서 사건사고가 일어나지 않는다면 신고를 하지 마십시오. 세이브의 내용이 규칙을 위반하거나 커뮤니티에 문제를 일으키는 것 같다면 신고하십시오! 여러분이 좋은 의도로 신고한다면 여러분이 이용 정지될 일은 절대 없습니다.\n"

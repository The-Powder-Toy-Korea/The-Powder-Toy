	...
	ui::Window(ui::Point(-1, -1), ui::Point(WINDOWW, WINDOWH))
{
	...
	addHeader("The Powder Toy는 커뮤니티 구성원들이 개발한 오픈 소스 프로젝트입니다.\n"
			"\bt{a:https://github.com/The-Powder-Toy/The-Powder-Toy|GitHub 저장소}\x0E에 기여해 주신 모든 분들께 감사드립니다:", false);
	...
	addHeader("스태프 - 커뮤니티와 웹사이트를 운영하는 봉사자");
	...
	addHeader("The Powder Toy의 한국어 번역자 - \bt{a:https://cafe.naver.com/powdertoy|The Powder Toy 공식 네이버 카페}\n");

	auto Korean = root["Korean"];
	for (auto &item : Korean)
	{
		ByteString username = item.asString();
		auto components = AddCredit(username.FromUtf8(), "", Small, "");
		organizeComponents(components, scrollPanel->Size.X);
	}

	for (int i = 0; i < 9; ++i)
	{
		auto components = AddCredit(" ", "", Small, "");
		organizeComponents(components, scrollPanel->Size.X);
	}
	...
	auto *closeButton = new ui::Button({ 0, Size.Y - 12 }, { Size.X, 12 }, "닫기");

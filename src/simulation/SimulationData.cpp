...
std::vector<wall_type> LoadWalls()
{
	...
	std::vector<wall_type>{
		{PIXPACK(0x808080), PIXPACK(0x000000), 0, Renderer::WallIcon, String("ERASE"),           "DEFAULT_WL_ERASE",  String("벽 지우개: 벽을 지웁니다.")},
		{PIXPACK(0xC0C0C0), PIXPACK(0x101010), 0, Renderer::WallIcon, String("CONDUCTIVE WALL"), "DEFAULT_WL_CNDTW",  String("전도성 벽: 전류를 제외한 모든 것들을 막습니다.")},
		{PIXPACK(0x808080), PIXPACK(0x808080), 0, Renderer::WallIcon, String("EWALL"),           "DEFAULT_WL_EWALL",  String("전원식 벽: 전류가 통하면 물질이 지나갈 수 있습니다.")},
		{PIXPACK(0xFF8080), PIXPACK(0xFF2008), 1, Renderer::WallIcon, String("DETECTOR"),        "DEFAULT_WL_DTECT",  String("감지기: 물질이 안에 있을 때 전류를 방출합니다.")},
		{PIXPACK(0x808080), PIXPACK(0x000000), 0, Renderer::WallIcon, String("STREAMLINE"),      "DEFAULT_WL_STRM",   String("흐름선: 흐름선의 시작점을 설정합니다.")},
		{PIXPACK(0x8080FF), PIXPACK(0x000000), 1, Renderer::WallIcon, String("FAN"),             "DEFAULT_WL_FAN",    String("선풍기: 공기 흐름을 가속합니다. 선 도구로 방향과 강도를 정할 수 있습니다.")},
		{PIXPACK(0xC0C0C0), PIXPACK(0x101010), 2, Renderer::WallIcon, String("LIQUID WALL"),     "DEFAULT_WL_LIQD",   String("액체 통과 벽: 액체 물질을 제외한 모든 물질을 막으며, 전도성이 있습니다.")},
		{PIXPACK(0x808080), PIXPACK(0x000000), 1, Renderer::WallIcon, String("ABSORB WALL"),     "DEFAULT_WL_ABSRB",  String("흡수 벽: 물질을 흡수하지만 공기 흐름은 통과시킵니다.")},
		{PIXPACK(0x808080), PIXPACK(0x000000), 3, Renderer::WallIcon, String("WALL"),            "DEFAULT_WL_WALL",   String("벽: 모든 것들을 막습니다.")},
		{PIXPACK(0x3C3C3C), PIXPACK(0x000000), 1, Renderer::WallIcon, String("AIRONLY WALL"),    "DEFAULT_WL_AIR",    String("압력 벽: 물질은 막지만 공기 흐름은 통과시킵니다.")},
		...
		{PIXPACK(0xFFAA00), PIXPACK(0xAA5500), 4, Renderer::WallIcon, String("ENERGY WALL"),     "DEFAULT_WL_ENRGY",  String("에너지 벽: 에너지 입자를 제외한 모든 입자들을 막습니다.")},
		{PIXPACK(0xDCDCDC), PIXPACK(0x000000), 1, Renderer::WallIcon, String("AIRBLOCK WALL"),   "DEFAULT_WL_NOAIR",  String("압력 막이 벽: 물질은 통과시키지만 공기 흐름은 막습니다.")},
		{PIXPACK(0x808080), PIXPACK(0x000000), 0, Renderer::WallIcon, String("ERASEALL"),        "DEFAULT_WL_ERASEA", String("지우기 도구: 벽과 입자, 물질 및 설명 등을 모두 지울 수 있습니다.")},
		{PIXPACK(0x800080), PIXPACK(0x000000), 0, Renderer::WallIcon, String("STASIS WALL"),     "DEFAULT_WL_STASIS", String("정지 벽: 전원이 공급될 때까지 자기 자신에 닿거나 자기 자신의 내부에 있는 모든 입자를 정지시킵니다.")},
	};
}
